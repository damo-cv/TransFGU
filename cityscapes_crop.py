...
def load_dataset_cityscapes(cfg, split='val', sample_range=None):
...
@ex.capture
def crop_and_extract_features_cityscapes_slide_window(cfg, model, model_val, data_loader, device, feat_save_dir, _log):
    batch_time = AverageMeter()
    tic = time.time()
    model.eval()

    batch_size_crops = cfg.dataset.train_batch_size // 2
    anchors = generate_box_anchors_by_scale(cfg.dataset.resize // cfg.model.encoder.patch_size,
                                            (cfg.dataset.resize*2) // cfg.model.encoder.patch_size,
    ...
    for index, sample in enumerate(data_loader):
        ...
        for image, sample_name in zip(images, sample_names):
            ...
            feat_save_path = os.path.join(feat_save_dir, sample_name+f"_cls_feat_{N_cls}_{h_crop}x{w_crop}")
            cls_feat = y_cls.cpu().numpy()
            dictfile = open(feat_save_path, 'wb')
            pickle.dump(cls_feat, dictfile)
            dictfile.close()
...
def main(_run, _log):
    ...
    with torch.no_grad():
        ...
        dataset_cityscapes = load_dataset_cityscapes(cfg, split=cfg.dataset.split, sample_range=sample_range)
        loader_cityscapes = DataLoader(dataset_cityscapes, batch_size=1, shuffle=False, num_workers=cfg.dataset.num_workers, pin_memory=True)

        model_val = model.module if hasattr(model, 'module') else model
        with torch.no_grad():
            crop_and_extract_features_cityscapes_slide_window(cfg, model, model_val, loader_cityscapes, device, feature_bank_dir, )