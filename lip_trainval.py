...
from dataloaders.lip_id_idx_map import lip_id_idx_coarse_16_map, lip_id_idx_coarse_5_map
from utils.misc import AverageMeter, init_process, sync_model, get_params_groups, save_network_checkpoint, load_network_checkpoint
...
@ex.capture
def load_train_dataset_lip(cfg, split='train', pseudo_label_save_dir=None, num_samples=None, _log=None):
...
def load_val_dataset_lip(cfg, split='val', pseudo_label_save_dir=None, num_samples=None):
    resize = 320
    train_transform = pth_transforms.Compose([
        transforms_uss_thingstuff.NormInput(),
        transforms_uss_thingstuff.ToTensor(),
        transforms_uss_thingstuff.ResizeTensor(size=(resize*2, resize), img_only=False),
    ])
...
class lip(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.pseudo_label_save_dir is not None:
            pseudo_label_save_path = os.path.join(self.pseudo_label_save_dir, sample_name +f'_{self.num_things}_{self.pseudo_label_size*2}x{self.pseudo_label_size}')
        ...
        if self.num_things == 5:
            _cityscape_id_idx_map = np.vectorize(lambda x: lip_id_idx_coarse_5_map[x])
            labels = _cityscape_id_idx_map(labels)
        elif self.num_things == 16:
            _cityscape_id_idx_map = np.vectorize(lambda x: lip_id_idx_coarse_16_map[x])
            labels = _cityscape_id_idx_map(labels)
        elif self.num_things == 19:
            pass
        else:
...
@ex.capture
def train_lip(cfg, model, model_init_weights, optimizer, data_loader, history, device, epoch, epoch_iter, _log):
    ...
    for index, sample in enumerate(data_loader):
        ...
        assert cfg.model.decoder.n_things == pseudo_label.shape[1]
        assert cfg.model.decoder.n_stuff == 1
        optimizer.zero_grad()
...
@ex.capture
def eval_lip(cfg, model, data_loader, history, device, epoch, exp_ckpt_dir, _log):
    batch_time = AverageMeter()
    tic = time.time()
    model.eval()
...
def main(_run, _log):
    ...
    pseudo_label_train_save_dir = os.path.join(cfg.dataset.root_dir_pseudo_label, f'pseudo_label_lip_{cfg.model.decoder.n_things}')
    ...
    train_dataset_lip = load_train_dataset_lip(cfg, split=cfg.dataset.split, pseudo_label_save_dir=pseudo_label_train_save_dir)
    if cfg.dataset.repeat > 0:
        train_dataset_lip = ConcatDataset([train_dataset_lip for _ in range(cfg.dataset.repeat)])
    val_dataset_lip = load_val_dataset_lip(cfg, split='val', pseudo_label_save_dir=None)
    val_loader_lip = DataLoader(val_dataset_lip, batch_size=cfg.dataset.val_batch_size,
                                 shuffle=False, num_workers=cfg.dataset.num_workers, pin_memory=True)
    if torch.cuda.is_available():
        val_loader_lip = PrefetchLoader(val_loader_lip)

    history = {'train': {'loss': [], 'metric_pred': 0, 'metric_pseudo_label': 0, },
               'val': {'metric': [], 'best_metric': 0}}
    if cfg.eval_only:
        with torch.no_grad():
            eval_lip(cfg, model, val_loader_lip, history, device, 0, exp_ckpt_dir)
    else:
        epoch_iter = 0
        for epoch in range(cfg.num_epochs):

            sampler = DistributedSampler(train_dataset_lip, shuffle=True) if torch.cuda.device_count() > 1 and use_ddp else None
            ...
            epoch_iter = train_lip(cfg, model, model_init_weights, optimizer, train_loader, history, device, epoch, epoch_iter, )

            if (epoch + 1) % cfg.eval_interval == 0:
                with torch.no_grad():
                    eval_lip(cfg, model, val_loader_lip, history, device, epoch, exp_ckpt_dir, )
